...
# this is adapted from the standard TCE model except that
# the 153 cap has affinity to the 153 head
# the 153 mask has low masking effect on the 305 head
# tumor infiltrating T cells are fewer
...
class transform:
  ...
  def __call__(self, system, t):
    if self.system is not system:
      ...
      self.analyteses_ = []
      self.analyteses_.append([system.analytes.index(f"{drug}") for drug in drugs] + [system.analytes.index(cap) for cap in caps])
      self.analyteses_.append([system.analytes.index(f"{drug}-A") for drug in drugs] + [system.analytes.index(cap) for cap in caps])
      self.analyteses_.append([system.analytes.index(f"{drug}-B") for drug in drugs] + [system.analytes.index(cap) for cap in caps])
      self.analyteses_.append([system.analytes.index(f"{drug}-AB") for drug in drugs] + [system.analytes.index(cap) for cap in caps])
      self.analyteses_.append([system.analytes.index(f"C-{drug}") for drug in drugs] + [system.analytes.index(cap) for cap in caps])
      self.analyteses_.append([system.analytes.index(f"C-{drug}-A") for drug in drugs] + [system.analytes.index(cap) for cap in caps])
      self.analyteses_.append([system.analytes.index(f"C-{drug}-B") for drug in drugs] + [system.analytes.index(cap) for cap in caps])
      self.analyteses_.append([system.analytes.index(f"C-{drug}-AB") for drug in drugs] + [system.analytes.index(cap) for cap in caps])
...
UT44.update({"density_cell": 3e8 * 0.44 / units.ml, "density_T": 3e8 * 0.015 / units.ml, "density_NK": 3e8 * 0.02 / units.ml})
...
FTC238.update({"density_cell": 3e8 * 0.44 / units.ml, "density_T": 3e8 * 0.015 / units.ml, "density_NK": 3e8 * 0.02 / units.ml})
...
def plot(system, name):
  ...
  groups = [antigens_effector,
            ...
            dimers_target,
            trimers]
  labels = ["effector", "target", "drug", "cap", "effector-drug", "drug-target", "effector-drug-target"]
  colors = ["tab:orange", "tab:blue", "black", "black", "wheat", "skyblue", "tab:purple"]
  linestyles = ["solid", "solid", "solid", "dotted", "solid", "solid", "solid", "solid"]
  ...
  groups = [[analyte for analyte in analytes if re.search("[mnc][mn]", analyte)],
            ...
            [analyte for analyte in analytes if re.search("[c][mn]", analyte)],
            [analyte for analyte in analytes if re.search("[mnc][n]", analyte)]]
  labels = ["(effector)-xx-(target)", "(effector)-nx-(target)", "(effector)-cx-(target)", "(effector)-xn-(target)"]
  ...
  groups = [["A"], ["B"],
            [analyte for analyte in analytes if re.match("[mnc][mn]-A$", analyte)],
            [analyte for analyte in analytes if re.match("[mnc][mn]-B$", analyte)],
            [analyte for analyte in analytes if re.match("[mnc][mn]-AB$", analyte)],
            [analyte for analyte in analytes if re.search("-[mnc][mn]-A", analyte)],
            [analyte for analyte in analytes if re.search("-[mnc][mn]-B", analyte)],
            [analyte for analyte in analytes if re.search("-[mnc][mn]-AB", analyte)]]
