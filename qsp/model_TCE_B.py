...
def model(TCE, plasma, lymph, organs):
  ...
  for drug in drugs:
    ...
    for organ in centrals + organs:
      ...
      system.add_simple(organ["name"], ["[T]C", f"H-{drug}"], [f"[T]C-{drug}", "H"], on_C, off_C / (100*units.nM))
      system.add_simple(organ["name"], ["[B]A", f"H-{drug}"], [f"[B]A-{drug}", "H"], on_A, off_A / (100*units.nM))
