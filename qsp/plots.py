...
midpoint = 10
maxpoint = 400
norm = mcolors.FuncNorm((lambda x: x/(x+midpoint), lambda y: midpoint*y/(1-y)), vmin=0, vmax=maxpoint)
...
def heatmap(df, xcol, ycol, xlabel, ylabel, zcols, zlabels, norm = norm, cmap = cmap, round = 2, output = None):
  fig, axs = plt.subplots(1, len(zcols), squeeze = False); axs = axs[0, :]
  for ax, zcol, zlabel in zip(axs, zcols, zlabels):
    ...
    for i in range(data.shape[1]):
      for j in range(data.shape[0]):
        text = ax.text(i, j, np.round(data.iloc[j, i], round), ha="center", va="center", color="w", fontsize = 6)
