...
def model(TCE, plasma, lymph, tumors, organs, connect_tumors = True):
  ...
  for drug in [f"{c}{a}{b}" for c in ("m", "n") for a in ("m", "n") for b in ("m", "n")]:
    off_P = TCE["off_P"]; on_P = {"n": 0 / units.nM / units.h, "m":TCE["off_P"] / TCE["aff_P"]}[drug[0]]
  ...
  for tumor in tumors:
    system.add_x("P", tumor["name"], 30000 * tumor["density_T"] / tumor["volume_interstitial_proportion"] / units.avagadro)
  ...
  for organ in organs:
    system.add_x("P", organ["name"], 10000 * organ["num_T"] / organ["volume_interstitial"] / units.avagadro)
